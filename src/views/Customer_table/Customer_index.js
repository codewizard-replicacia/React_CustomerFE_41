...
function CustomerTable() {
  ...
  const columns = [
    { title: "CustId", field: "CustId", editable: "never" },
      { title: "CustAccountNumber", field: "CustAccountNumber" },
  ...
  const fetchData = async (query) => {
    return new Promise(async (resolve, reject) => {
      ...
      const url = `${BASE_URL}${PATH_CUSTOMER}`;
      ...
      const countUrl = search ? `${url}/$count?${filterQuery}` : `${BASE_URL}${PATH_CUSTOMER}/$count`;
      let total = null;
      ...
      <MuiTable
        ...
        actions={[
          ...
          },
          {
            ...
            navigate(`/Customers/view/${rowData.CustId}`),
          },
          {
            icon: Edit,
            tooltip: "Edit",
            onClick: (event, rowData) =>
            navigate(`/Customers/edit/${rowData.CustId}`),
          },
        ]}
        onRowDelete={async (oldData) => {
          const resp = await makeApiCall(
            `${BASE_URL}${PATH_CUSTOMER}(${oldData.CustId})`,
            "DELETE"
          );
          if (resp.ok) {
            tableRef.current.onQueryChange();
            snackbar.enqueueSnackbar("Successfully deleted Customers", {
              variant: "success",
            });
          } else {
